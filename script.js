document.addEventListener('DOMContentLoaded', function() {
    ...
    if (contactForm) {
        ...
        contactForm.addEventListener('submit', function(e) {
            ...
            const inputGroups = contactForm.querySelectorAll('.form-group, .input-group');
            ...
            requiredFields.forEach(field => {
                ...
                const fieldName = field.getAttribute('name') || field.id || 'This field';
            ...
            // Validate textarea if present
            const textareas = contactForm.querySelectorAll('textarea[required]');
            textareas.forEach(textarea => {
                const value = textarea.value.trim();
                const fieldName = textarea.getAttribute('name') || textarea.id || 'This field';
                
                if (!value) {
                    isValid = false;
                    showFieldError(textarea, `${fieldName} is required`);
                } else if (value.length < 10) {
                    isValid = false;
                    showFieldError(textarea, `${fieldName} must be at least 10 characters`);
                } else {
                    formData[fieldName] = value;
                }
            });
        ...
        contactForm.addEventListener('blur', function(e) {
            ...
            const value = field.value.trim();
            const parentGroup = field.closest('.form-group, .input-group');
            
            if (value && field.type === 'email' && !isValidEmail(value)) {
                if (parentGroup) parentGroup.classList.add('error');
            } else if (value && field.type === 'tel' && !isValidPhone(value)) {
                if (parentGroup) parentGroup.classList.add('error');
            } else if (value) {
...
/* SIGNATURE MOMENT: scroll-driven-hero */
(function(){

      (function(){
        if (window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
        var hero = document.querySelector('main > section, .hero-section, [data-band-index="0"]');
        if (!hero) return;
        // SKIP pinned / sticky-scroll sections. A pinned-scrollytelling block keeps
        // its media column sticky while the text column scrolls; layering the hero
        // fade on top dims that scrolling copy to ~15% opacity mid-scroll, making the
        // last paragraphs unreadable. Detect by class first (cheap, reliable), then
        // fall back to a bounded computed-style scan for any position:sticky child.
        if (/\b(?:pinned-section|pinned-scrollytelling|sticky-section)\b/.test(hero.className || '')) return;
        var sticky = hero.querySelector('.pinned-media, .sticky-media, [class*="pinned"], [class*="sticky"]');
        if (!sticky) {
          var kids = hero.querySelectorAll('*');
          for (var k = 0; k < kids.length && k < 120; k++) {
            if (getComputedStyle(kids[k]).position === 'sticky') { sticky = kids[k]; break; }
          }
        }
        if (sticky) return;
        var inner = hero.querySelector('.hero-content, .hero-inner, .container') || hero;
        hero.classList.add('zsm-sdh-target');
        var ticking = false;
        function update(){
          var rect = hero.getBoundingClientRect();
          var vh = window.innerHeight || document.documentElement.clientHeight;
          var progress = Math.min(1, Math.max(0, -rect.top / vh));
          inner.style.transform = 'translate3d(0,' + (progress * 60).toFixed(1) + 'px,0) scale(' + (1 - progress * 0.06).toFixed(4) + ')';
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '9a79029f-d22d-4395-936e-1cc6b7f088ec',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '9a79029f-d22d-4395-936e-1cc6b7f088ec';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '9a79029f-d22d-4395-936e-1cc6b7f088ec';
